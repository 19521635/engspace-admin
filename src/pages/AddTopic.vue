		...
		<div class="row">
			...
			<div class="col-12 col-xl-4">
				<div class="card-content overflow-hidden p-4 shadow-sm">
					...
					<div>
						<div @click="$router.push({ name: 'topic_detail', params: { id: item.id } })" class="btn btn-light w-100 text-left mb-2 p-2" v-for="item in rows.slice().reverse()" v-bind:key="item">
							{{ item.name }}
						</div>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			submitForm() {
				TopicDataService.post(this.form)
					.then((response) => {
						this.form = response.data;
						this.$router.push({ name: "topic_detail", params: { id: this.form.id } });
					})
					.catch((err) => {
						console.log(err);
					});
