		...
		<div class="card-content overflow-hidden p-4 shadow-sm">
			<div class="d-flex flex-row align-items-center mb-4">
				...
				<div class="d-flex flex-row w-100 justify-content-end">
					...
					<button class="btn btn-info" @click="$router.push({ name: 'add_comment' })">Thêm mới</button>
				</div>
			</div>
			<Table :list="rows" @deleteById="deleteById" @editById="editById" :listDisplay="['id', 'post', 'parent', 'date_created', 'date_updated']" v-if="rows.length"></Table>
...
<script>
	...
	export default {
		...
		methods: {
			retrieveComments() {
				CommentDataService.get(this.search, this.currentPage, this.pageSize)
					.then((response) => {
